...
function insertar_datos($cod, $nombreA, $PiApe, $SeAp, $correoA, $carrera, $pass, $tipo, $semestre)
{
    ...
    //$tutoralumno = "insert into relaciontutoralumno (codigoAlumno, correoTutores) values ($cod,  )";
...
function binarySearchFor($array, $left, $rigth, $objetivo){
    if($left >= $rigth){
        return false;
    }
    $mid = ($left + $rigth) / 2;
    if($array[$mid] > $objetivo){
        binarySearchFor($array, $left, $mid, $objetivo);
    }else if($array[$mid] < $objetivo){
        binarySearchFor($array, $mid, $rigth, $objetivo);
    }else{
        return true;
    }
}
